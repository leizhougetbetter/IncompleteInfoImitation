...
#
#
function sImitationUpdate(nodeChosenUpdate, numNeighChosenArray, strategyMain, gamePayoff, networkStructure, selectionIntensity)
    ###
    ...
    #Consider personal information or not
    ...
    sumCoopFitness += strategyNodeChosenUpdate * fitnessFocalPlayer
    sumDefectFitness += (1-strategyNodeChosenUpdate) * fitnessFocalPlayer
    ...
    for iNeighbor in 1 : numNeighChosen
        ...
        if strategyNeighborNow == 1
            sumCoopFitness += fitnessNeighborNow
        elseif strategyNeighborNow == 0
            sumDefectFitness += fitnessNeighborNow  
        end
    end
...
#
function calculateFPCorD(numRepetition, numGeneration, networkStructure, gamePayoff, selectionIntensity, numNeighChosen)
    ####
    ...
    for iRepetition in 1 : numRepetition
        ...
        for iGeneration in 1 : numGeneration
            ...
            strChangeDirectionCurrent = sImitationUpdate(nodeChosenUpdate, numNeighChosen, strategyMain, gamePayoff, networkStructure, selectionIntensity)
...
#
#
function mainProgramCalculateFpCandD(pairwiseGamePars::Tuple{Float64, Float64, Float64, Float64}, invasionTrialsPars::Tuple{Int,Int,Float64}, fileIOPars::NTuple{3,String}, numNeighChosenMain)
        ...
        for indSS in 1 : length(SSValue)
            ...
            for rhoCorRhoD in 0 : 1
                ...
                if rhoCorRhoD == 0
                    ...
                    fpCorD, invasionTimes, extinctionTimes, coexistenceTimes = calculateFPCorD(numRepetition, numGeneration, networkStructureMain, gamePayoff, selectionIntensity, numNeighChosenMain)
                    # Juno.@profiler fpCorD, invasionTimes, extinctionTimes, coexistenceTimes = calculateFPCorD(updateFunctionMain, numRepetition, numGeneration, networkStructureMain, gamePayoff, selectionIntensity, gameTransitionProb)
                elseif rhoCorRhoD == 1
                    ### calculate rho_B
                    ##for pairwise social dilemmas
...
inputArgsArray = Vector(undef, 10)
...
for indGraph = indGraphBeg : indGraphEnd
    ...
    mainProgramCalculateFpCandD(pairwiseGameParsInput, invasionTrialsParsInput, fileIOParsInput, numNeighChosenMain)
